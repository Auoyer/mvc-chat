function init_control() {
    $("#zone_left").height(window.innerHeight), $("#zone_left .list-group").height(window.innerHeight - 225), $("#div_msgpanel").width(.45 * (window.innerWidth - 100)), $("#div_msgpanel").height(window.innerHeight - 200), $("#div_msgbox").height(window.innerHeight - 274), $("#div_privmsg").height(window.innerHeight - 274), $("#nav_action").width(window.innerWidth - $("#div_msgpanel").width() - $("#zone_left").width() - 40), $("#zone_left").show(), $("#zone_center").show(), 1 != $.cookie("isSound") && $.cookie("isSound") ? $("#msg_tips").attr("src", "IMG/sound_off.png") : $("#msg_tips").attr("src", "IMG/sound_on.png")
}

function fun_getnickname(a) {
    $.cookie("user_id") && $.cookie("user_nickname") && a ? (ws || fun_initWebSocket(), $("#inp_nickname").val($.cookie("user_nickname"))) : $.getJSON("http://115.29.193.48:28001/Act/h_main.ashx?callback=?", {
        act: "getRanNick"
    }, function (b) {
        "OK" == b.state ? ($.cookie("user_id") || $.cookie("user_id", b.msg.id, {
            expires: 365
        }), $.cookie("user_nickname", b.msg.nickname, {
            expires: 365
        }), $("#inp_nickname").val(b.msg.nickname), !a && ws && 1 == ws.readyState && sendJson("chgname", "", !0), ws || fun_initWebSocket()) : layer.msg(b.msg, {
            icon: 6
        })
    })
}
...
function fun_initWebSocket() {
    return isconning ? (layer.msg("正在连接,请稍后..."), void 0) : (isconning = !0, ws = new WebSocket(ws_addr), ws.onopen = function () {
        isconning = !1, $("#btn_conn").attr("disabled", !0), layer.msg("欢迎登录~"), sendJson("sign", "", !0)
        ...
        switch (b.code) {
            case -1:
                ...
                break;
            case -2:
                isforceOut = !0, layer.alert(b.content, {
                    icon: 5,
                    title: "离线"
                });
                break;
        ...
        isconning = !1, $("#btn_conn").attr("disabled", !1), isforceOut || layer.alert("连接断开...", {
            icon: 5,
            title: "提示"
...
function fun_bindList(a) {
    var d, b = a.users_list,
		c = "";
    for (d = 0; d < b.length; d++) b[d].id != $.cookie("user_id") && (dic_userlist[b[d].id] = b[d].nickname, c += "<a id='userid_" + b[d].id + "' class='list-group-item' onfocus='this.blur()' onclick='click_userlist(id);'><span class='badge'></span><span style='margin-right:5px;' class='glyphicon glyphicon-user'></span><span class='nickname'>" + b[d].nickname + "<span></a>");
    $("#user_list").html(c), $("#nowusers_count").text(b.length)
}

function fun_newuser(a) {
    dic_userlist[a.user.id] || ($("#user_list").prepend("<a id='userid_" + a.user.id + "' class='list-group-item' onfocus='this.blur()' onclick='click_userlist(id);'><span class='badge'></span><span style='margin-right:5px;' class='glyphicon glyphicon-user'></span><span class='nickname'>" + a.user.nickname + "</span></a>"), dic_userlist[a.user.id] = a.user.nickname, $("#nowusers_count").text(parseInt($("#nowusers_count").text()) + 1))
}
...
function fun_userchgname(a) {
    dic_userlist[a.user.id] && (dic_userlist[a.user.id] = a.user.nickname, $("#userid_" + a.user.id + " .nickname").text(a.user.nickname)), $("#div_msgbox").append('<div class="chat_msg"><div>' + a.user.time + "</div>" + a.user.beforename + "->" + a.user.nickname + "</div>"), $("#div_msgbox").scrollTop($("#div_msgbox")[0].scrollHeight)
}

function fun_recbrodata(a) {
    haveTitleTips("新消息"), a.user.id == $.cookie("user_id") && (pub_lastSendTime = a.user.time), $("#div_msgbox").append(fun_chatbox(a.user, a.code)), $("#div_msgbox").scrollTop($("#div_msgbox")[0].scrollHeight)
}
...
function click_userlist(a) {
    a = a.split("_")[1], sel_userid != a ? ($("#userid_" + sel_userid).removeClass("active"), $("#userid_" + a).addClass("active"), $("#msg_title").html('和[<span style="color:orange">' + dic_userlist[a] + "</span>]的聊天记录"), sel_userid = a, ws && 1 == ws.readyState && (fun_loading(!0), sendJson("getprivmsg", sel_userid, !0), $("#userid_" + sel_userid + " .badge").text(""))) : fun_showPublicZone()
}
...
function fun_chatbox(a, b) {
    ...
    switch (a.code && (b = a.code), b) {
        case 4:
            ...
            break;
        default:
            d = a.id == $.cookie("user_id"), e = 8 == b || 7 == b ? $.cookie("user_id") : a.id, f = d ? "<span style='color:#d2d2d2;'>" + a.time + "</span> <span style='font-weight:900;color:#6d6d6d'>" + a.nickname + "</span>" : "<span style='font-weight:900;color:#6d6d6d'>" + a.nickname + "</span> " + "<span style='color:#d2d2d2;'>" + a.time + "</span>", c = "<div class='chatbox' style='" + (d ? "float:right" : "float:left") + "' ><div style='text-align:" + (d ? "right" : "left") + "'>" + f + "</div>" + "<div class='chatarrow' style='" + (d ? "right:5px;border-bottom: 8px solid #ffe6b8;" : "left:5px;border-bottom: 8px solid #cfffcf;") + "'></div><div id='" + (sortID(e) + a.time.replace(/:/g, "_")) + "' class='chat" + (d ? " selfchat" : "") + "' style='max-width:" + ($("#div_msgpanel").width() - 50) + "px;'>" + chg_emoji(a.content, a.id) + "<br />" + "</div>" + "</div><div class='clearboth'></div>"
    }
    return c
}
...
function haveTitleTips(a) {
    isFocus || tick_titletips ? isFocus || haveSoundTips() : (haveSoundTips(), tick_titletips = setInterval(function () {
        document.title = -1 == document.title.indexOf(a) ? "[" + a + "]-来聊吧" : "[　　　]-来聊吧", isFocus && (clearInterval(tick_titletips), tick_titletips = null, document.title = "来聊吧")
    }, 500))
}
...
function mouseover_zoomimg(a) {
    var b, c;
    0 == index_zoomimg && (b = a.split("_")[1], c = a.split("_")[2], index_zoomimg = layer.tips("<div><img style='max-width:380px;' src='IMG/Upload/" + $.cookie("user_id") + "/" + b + "." + c + "' /></div>", "#" + a, {
        tipsMore: !0,
        tips: [2, "#fff"],
        time: 0,
        area: "auto",
...
function mouseout_closezoom() {
    layer.close(index_zoomimg), index_zoomimg = 0
}
...
function result_getphoto(a) {
    ...
    var b = $.parseJSON(a);
    "Fail" != b.state ? ($("#inp_say").val("[" + b.msg + "]"), $("#btn_say").trigger("click")) : layer.msg("上传照片失败...")
}
...
function voice_errordialog(a) {
    layer.close(index_getvoice), layer.msg(a)
}

function result_getvoice(a) {
    layer.close(index_getvoice), "Fail" != a.state ? ($("#inp_say").val("[" + a.msg + "]"), $("#btn_say").trigger("click")) : layer.msg(a.msg)
}
...
function uploadfile_errordialog(a) {
    layer.msg(a)
}

function result_uploadfile(a) {
    layer.close(index_uploadfile), "Fail" != a.state ? ($("#inp_say").val("[" + a.filename + "|" + a.filesize + "|" + a.msg + "]"), $("#btn_say").trigger("click")) : layer.msg(a.msg)
}
...
function result_doodle(a) {
    ...
    var b = $.parseJSON(a);
    "Fail" != b.state ? ($("#inp_say").val("[" + b.msg + "]"), $("#btn_say").trigger("click")) : layer.msg(b.msg)
}
var ws_addr, ws, tick_heartpac, tick_titletips, isconning, isforceOut, isFocus, pub_lastSendTime, pri_lastSendTime, emoji, dic_userlist, sel_userid, isloading, index_loading, isopenEmoji, index_emoji, uploadPic, isopenImg, index_img, index_zoomimg, isBeforeUpload, index_getphoto, index_getvoice, index_uploadfile, index_doodle, swfobject = function () {
    function A() {
        var a, c, d;
        if (!t) {
            try {
    ...
    return function () {
        ...
        embedSWF: function (c, d, e, f, g, h, i, j, k, l) {
            ...
            y.w3 && !(y.wk && y.wk < 312) && c && d && e && f && g ? (U(d, !1), B(function () {
                var n, o, p, q, r, s;
                if (e += "", f += "", n = {}, k && typeof k === b) for (o in k) n[o] = k[o];
                if (n.data = c, n.width = e, n.height = f, p = {}, j && typeof j === b) for (q in j) p[q] = j[q];
...
(function (a) {
    ...
    var b = /\+/g,
		h = a.cookie = function (b, f, i) {
		    ...
		    if (void 0 !== f && !a.isFunction(f)) return i = a.extend({}, h.defaults, i), "number" == typeof i.expires && (j = i.expires, k = i.expires = new Date, k.setDate(k.getDate() + j)), document.cookie = [c(b), "=", e(f), i.expires ? "; expires=" + i.expires.toUTCString() : "", i.path ? "; path=" + i.path : "", i.domain ? "; domain=" + i.domain : "", i.secure ? "; secure" : ""].join("");
...
ws_addr = "ws://115.29.193.48:29000", ws = null,
tick_heartpac = null,
tick_titletips = null,
isconning = !1,
isforceOut = !1,
...
$(function () {
    ...
    $("#btn_say").click(function () {
        ...
        if (ws && 1 == ws.readyState) {
            ...
            sel_userid ? sendJson("touser_" + sel_userid + "_" + dic_userlist[sel_userid], a, !0) : sendJson("toall", a, !0), $("#inp_say").val("")
        ...
        var a = layer.confirm("是否确定清除记录？", {
            ...
            layer.close(a), $("#div_msgbox").html("")
        })
    }), $("#msg_back").click(function () {
        var b, a = sel_userid ? sel_userid + "_" + pri_lastSendTime[sel_userid] : pub_lastSendTime;
        ws && 1 == ws.readyState && a && (b = layer.confirm("是否确定撤回最新消息？", {
            skin: "demo-class",
            btn: ["确定", "取消"]
        }, function () {
            -1 != a.indexOf("_") ? pri_lastSendTime[sel_userid] = "" : pub_lastSendTime = "", layer.close(b), fun_loading(!0), sendJson("msgrevoke", a, !0)
        }))
    })
}),
...
, isloading = null, isopenEmoji = !1, index_emoji = 0, uploadPic = [], isopenImg = !1, index_img = 0, index_zoomimg = 0, isBeforeUpload = !1;